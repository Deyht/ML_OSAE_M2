...
cnn.conv_create(f_size=i_ar([5,5,1]), nb_filters=6, stride=i_ar([1,1,1]), padding=i_ar([2,2,0]), activation="RELU")
...
cnn.pool_create(p_size=i_ar([2,2,1]), p_type="MAX")
cnn.dense_create(nb_neurons=128, activation="RELU", drop_rate=0.0)
cnn.dense_create(nb_neurons=64, activation="RELU", drop_rate=0.0)
cnn.dense_create(nb_neurons=10, activation="SOFTMAX")


cnn.train_network(nb_epoch=40, learning_rate=0.001, momentum=0.0, confmat=1, save_each=10000)
